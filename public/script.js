...
async function initializeSupabase() {
    try {
        ...
        // Configurer le canal de messages en temps réel
        messagesChannel = supabase
            .channel('quiz-messages')
            .on('postgres_changes', {
                event: 'INSERT',
                schema: 'public',
                table: 'messages'
            }, (payload) => {
                handleSupabaseMessage(payload.new);
            })
            .subscribe();
...
function handleSupabaseMessage(data) {
    console.log('📨 Message reçu:', data.type, 'de', data.email || 'unknown');
...
async function broadcastMessage(message) {
    try {
        if (supabase) {
            // Utiliser Supabase pour diffuser le message
